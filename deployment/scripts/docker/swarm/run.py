...
parser.add_argument(
    ...
    const="all",
    nargs="?",
    choices=["all", "collector", "parser", "persister", "reporter"],
...
def main():
    ...
    export_environment_variables(".env-doiex")
    ...
    if not args.skip_docker_deploy:
        if docker_swarm_is_runnig():
            ...
            docker_create_config(
                "traefik_toml",
                f"reverse-proxy/{os.environ['REVERSE_PROXY_CONFIG_FILE']}.toml",
            )
            generate_local_certificates(os.environ["CERTIFICATES_DIR"])
            docker_create_secret(
                "doiex_root_crt", f"{os.environ['CERTIFICATES_DIR']}/doiex.root.crt"
            )
            docker_create_secret(
                "doiex_crt", f"{os.environ['CERTIFICATES_DIR']}/doiex.crt"
            )
            docker_create_secret(
                "doiex_key", f"{os.environ['CERTIFICATES_DIR']}/doiex.key"
            )
            docker_stack_deploy(
                "docker-swarm/docker-compose-reverse-proxy.yml", reverse_proxy_service
            )

            doiex_services = "doiex_services"
            docker_remove_network(doiex_services)
            docker_create_network(doiex_services)

            doiex_database = "doiex_database"
            docker_remove_network(doiex_database)
            docker_create_network(doiex_database)

            docker_stack_deploy("docker-swarm/docker-compose-doiex.yml", "doiex")
